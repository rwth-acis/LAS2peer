 ...
 *
 * Requires Ethereum credentials with sufficient Ether funds to pay for transaction gas.
 *
...
public class ReadWriteRegistryClient extends ReadOnlyRegistryClient {
	 ...
	 *
	 * This will register the user (login) name to the agent's Ethereum account and store some additional fields that
	 * help others verify the user's identity. (E.g., the las2peer public key is stored in order to verify message
	 * signatures etc.)
	 *
	 * As with the other delegated registry methods, the registration transaction itself is performed by the node
	 * operator; but the agent's credentials are used to sign a message consenting to the registration. The registry
	 * smart contracts verify that signature and perform the registration as if the transaction came directly from the
	 * agent. (This simply means that the user name is registered to the agent's account, not the node operator's.)
	 *
	...
	public String addGenericTransaction(EthereumAgent senderAgent, EthereumAgent receivingAgent, String message,
		...
		// sendEther(senderAgent.getEthereumAddress(), receivingAgent.getEthereumAddress(),
		// weiAmount).getTransactionHash();
	 ...
	 *
	 * The registration call is delegated, see description of {@link #registerUser(EthereumAgent)} for details.
	 *
	 ...
	 *
	 * The release call is delegated, see description of {@link #registerUser(EthereumAgent)} for details.
	 *
	 ...
	 * 
	 * @param servicePackageName name of service (package). E.g. <code>com.example.some.package</code>.
	 * @param serviceClassName name of the service class that is run. E.g., <code>ExampleService</code>.
	 * @param versionMajor major version, as used in semantic versioning
	 * @param versionMinor minor version, as used in semantic versioning
	 * @param versionPatch patch version, as used in semantic versioning
	 ...
	 * 
	 * This sends ether from the "owner" of this registry client (i.e., the account specified by the credentials which
	 * were used to construct this client), to the given recipient address.
	 * 
	 * The calling agent (i.e. ethereumAgent vs ethereumNode) influences who pays for the transaction fees. In smart
	 * contracts, this would also mean who is msg.sender
	 * 
	 * @param senderAddress
